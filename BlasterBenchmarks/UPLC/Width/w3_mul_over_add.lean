...
theorem w3_mul_over_add : ∀ (x y z : Int), Uplc.run (.lam "x" (.lam "y" (.lam "z" (.app (.app (.bltn .Add) (.app (.app (.bltn .Mul) (.var "x")) (.var "y"))) (.app (.app (.bltn .Mul) (.var "x")) (.var "z")))))) [(.con x), (.con y), (.con z)] 60 = some (x*y + x*z) := by sorry
