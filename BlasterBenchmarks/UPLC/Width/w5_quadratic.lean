...
theorem w5_quadratic : ∀ (a x b c : Int), Uplc.run (.lam "a" (.lam "x" (.lam "b" (.lam "c" (.app (.app (.bltn .Add) (.app (.app (.bltn .Add) (.app (.app (.bltn .Mul) (.var "a")) (.app (.app (.bltn .Mul) (.var "x")) (.var "x")))) (.app (.app (.bltn .Mul) (.var "b")) (.var "x")))) (.var "c")))))) [(.con a), (.con x), (.con b), (.con c)] 120 = some (a*(x*x) + b*x + c) := by sorry
