...
import BlasterBenchmarks.UPLC.CekMachine
...
/-! ## Definition of #preprocess_uplc that optimizes a uplc code according to the provided script budget.
    ...
    E.g.
     Command `#prep_uplc "compiledProcessSCOrder" processSCOrder [ProcessSCInput] orderInputToParams 500`
     will generate the following two definitions
      - prop_compiledProcessSCOrder: corresponding to the optimized uplc code that can be used for formal proof.
      - exec_compiledProcessSCOrder: corresponding to the executable uplc code w.r.t. the cex machine.
-/
...
def preprocessImp : CommandElab := fun stx => do
  let (propDecl, execDecl) ←
      withoutModifyingEnv $ runTermElabM fun _ => do
        ...
        let propName := Name.mkStr1 (s!"prop_{baseName}")
        let execName := Name.mkStr1 (s!"exec_{baseName}")
        let propDef := createDefinition propName t e
        let execDef := createDefinition execName t app
        return (propDef, execDef)
